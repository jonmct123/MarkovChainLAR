...
num_cores = mp.cpu_count()-1   # find the number of available cores (also for parallelization)
import time
from scipy.integrate import quad # quad performs integrans over a single variable
import pandas as pd
...
def vec_Markov_parallel(abs_v,n_ref_L,dev_angle,QY_v,sun_v):
    ...
    t_c_P2L = np.arcsin(n_ref_L/n_ref_P)
...
eta_n = np.zeros((len(n_lst),len(abs_lst)))
eta_a = np.zeros((len(abs_lst),len(n_lst)))
eta_v = np.zeros(len(abs_lst))
eta_t = np.zeros((len(n_lst),len(tilt_lst)))
eta_q = np.zeros((len(n_lst),len(q_lst)))
eta_abs_iso = np.zeros((len(abs_lst),len(n_lst)))
eta_sun = np.zeros((len(n_lst),len(sun_lst)))

current_var = np.array([0]) ### Ideal

current_var_name = 'Auger'
